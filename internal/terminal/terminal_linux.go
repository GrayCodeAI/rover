...
func Raw(f *os.File) (func() error, error) {
	...
	if e := ioctl(f.Fd(), syscall.TCGETS, unsafe.Pointer(&old)); e != nil {
		return nil, e
	}
	n := old
	...
	if e := ioctl(f.Fd(), syscall.TCSETS, unsafe.Pointer(&n)); e != nil {
		return nil, e
	}
	return func() error { return ioctl(f.Fd(), syscall.TCSETS, unsafe.Pointer(&old)) }, nil
}
