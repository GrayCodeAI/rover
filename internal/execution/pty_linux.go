...
func openPTY() (*os.File, *os.File, error) {
	...
	var unlock int32
	var n uint32
	if e = ptyIoctl(m.Fd(), 0x40045431, unsafe.Pointer(&unlock)); e != nil {
		m.Close()
		return nil, nil, e
	}
	if e = ptyIoctl(m.Fd(), 0x80045430, unsafe.Pointer(&n)); e != nil {
		m.Close()
		return nil, nil, e
	}
	sl, e := os.OpenFile(fmt.Sprintf("/dev/pts/%d", n), os.O_RDWR|syscall.O_NOCTTY, 0600)
	if e != nil {
		m.Close()
		return nil, nil, e
	}
	w := [4]uint16{24, 80}
	_ = ptyIoctl(m.Fd(), syscall.TIOCSWINSZ, unsafe.Pointer(&w))
	return m, sl, nil
}
...
func (b *broker) serve(c net.Conn, ch chan terminalFrame, ring []byte, drained chan struct{}) {
	...
	for sc.Scan() {
		...
		case "resize":
			...
			if ptyIoctl(b.master.Fd(), syscall.TIOCSWINSZ, unsafe.Pointer(&w)) != nil {
				return
			}
		case "detach":
			return
